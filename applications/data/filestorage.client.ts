...
export class FileStorage implements IStorageClient {
    ...
    insert(submission: Submission): Promise<string> {
        const fileName = this._fileNameFromUrl(submission.targetUrl) + '_' + new Date().getTime() + '.json';

        return new Promise<string>((res, rej) => {

            fs.writeFile(path.join(this._directory, fileName), JSON.stringify(submission), { encoding: this._fileCharset }, err => {
                if (err) {
                  rej(err);
                }
